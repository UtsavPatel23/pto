		...
		axios.post(PRODUCT_ENQUIRY, enquiryData)
			.then(res => {
                ...
                setEnquiryFields({
                    ...enquiryFields,
                    enquiry_error: '',
                    enquiry_success: 'Enquiry successful',
                    enquiry_loading: false
                })
				reset();
			})
	...
	return (
		<React.Fragment>
			<div className='border border-gray-200 p-3 rounded'>
				{enquiry_error && <p className="bg-red-600 text-white p-2 text-center w-full text-xl" dangerouslySetInnerHTML={createMarkup(enquiry_error)} />}
				{enquiry_success && <p className="bg-green-600 text-white p-2 text-center w-full text-xl" dangerouslySetInnerHTML={createMarkup(enquiry_success)} />}
				<form onSubmit={handleSubmit(onFormSubmitenquiry)}>
					...
					<label className="block mb-4">
						...
						<input name="enquiry" type="text" {...register('enquiry')} className='outline-none block w-full py-2 px-3 text-base  border border-gray-300 focus:border-victoria-400' />
