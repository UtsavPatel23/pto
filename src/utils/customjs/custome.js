        ...
        var arr_msg = [];
        {
          ...
          options.discount_rate_product_quantity_discount.map((discount_item) => {
            ...
            if (item.quantity < discount_item.quantity) {
              ...
              arr_msg.push("<span className='variation-Note'>Note: </span> Add " + (discount_item.quantity - item.quantity) + " more to get " + discount_item.rate_percentage + "% discount " + cartItems?.[0]?.currency + dis_price.toFixed(2));
            } else {
              tmpDiscount = ((item.line_subtotal * discount_item.rate_percentage) / 100);
              msgTmpDiscount = "<span className='variation-Note'>Discount: </span>  " + (discount_item.rate_percentage) + "% discount applied (" + ((item.line_subtotal * discount_item.rate_percentage) / 100).toFixed(2) + ")";
            }
          })
        }
